...
def result():
    if request.method == 'POST':
        ...
        def getting_answer(final):
            ...
            now_date_a = datetime.now()
            now_date_a = now_date_a.strftime("%d/%m/%Y %H:%M:%S")
            filter_date_a = now_date_a

            answer_data = answer_data[answer_data['modified_on'] < filter_date_a]
        ...
        model = KeyedVectors.load_word2vec_format(
            'D:/ML/QNA_project/model/GoogleNews-vectors-negative300.bin/GoogleNews-vectors-negative300.bin',
            binary=True)
        # t2 = time.time()
        print('model loaded ')
        vs = vectors(words)
        ...
        dummy = dummy[dummy['modified_on'] < filter_date_q]
        ...
        dic = final.head().to_dict()
        print(time.time()-t1)

        return render_template('result.html' , answer = dic)
